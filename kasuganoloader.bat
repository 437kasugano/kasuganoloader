...
echo [1] OTCv3 + new models [CHECK NOTE]
echo [2] Osiris
echo [3] Weave 2.5
echo [4] RaweTrip
echo [5] Nixware CRACK [CHECK NOTE]
echo [6] Download CFGS
echo [7] VirusTotal scans
echo [8] Buy cheats (reselling services)
echo [9] Legacy HvH tab
echo [10] NOTE (old DLL's fix)
echo [11] Exit the loader
set /p car=
if %car% == 1 goto otc
...
:note
cls
start https://cdn.discordapp.com/attachments/969690219798495322/970002834315677787/csgo_previous_depot.zip
echo After CSGO got updated, every free/paid cheat got patched.
echo Since most of the free cheats now are not very popular it's very rare for them to get fixed
echo To use OTCv3 DLL that its not updated you need to download old CSGO files which download just started
echo Unzip every file from that zip into csgo installation directory and try injecting the cheats. should work
echo if its still not working, dm me 437Kasugano#0437 or open a ticket at my server https://discord.gg/2Ea5RUCDNr
echo [1] To go back
set /p car=
if %car% == 1 goto start